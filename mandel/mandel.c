...
// only makes i private
// eps is not being written, so it doesnt need to be private
#pragma omp parallel for default(shared) private(i, j)
    ...
    area =
        2.0 * 2.5 * 1.125 * (double)(NPOINTS * NPOINTS - numoutside) / (double)(NPOINTS * NPOINTS);
...
#pragma omp atomic
            // makes a copy of numoutside for each thread
            numoutside++;
            break;
        }
    }
